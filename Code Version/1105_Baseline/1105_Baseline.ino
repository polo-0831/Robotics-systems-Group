...
void loop()
{
  ...
  float normalizedLeftBumpValue = (leftBumpValue - LeftBump_Min) / (LeftBump_Max - LeftBump_Min);
  float normalizedRightBumpValue = (rightBumpValue - RightBump_Min) / (RightBump_Max - RightBump_Min);

  normalizedLeftBumpValue = constrain(normalizedLeftBumpValue, 0.0, 1.0);
  normalizedRightBumpValue = constrain(normalizedRightBumpValue, 0.0, 1.0);
  
  LeftCollide_thres = 0.75;
  RightCollide_thres = 0.75;
  ...
  */
  ...
  Serial.print("normalizedLeftBumpValue :");
  Serial.print(normalizedLeftBumpValue);
  Serial.print(",");
  Serial.println(normalizedRightBumpValue);
...
{
  float speed_demand = (-5 / 0.69 * bump_value) + (5 / 0.69);
  return speed_demand;
}
/*
float Bumpvalue2Speed(float bump_value)
